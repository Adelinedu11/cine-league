-- 025 — Ciné'Files : palier 15-20 essais → 10 points pour l'auteur (au lieu de 50)
-- Dépend de : 016, 017, 019, 020, 022, 024.
--
-- Seule la branche author_points `between 15 and 20` passe de 50 à 10. Le reste
-- est inchangé : devineur = compute_cine_score (1-14 formule, 15-20 → 10) ;
-- auteur = 100 - points_devineur (1-14) / 10 (15-20) / 0 (jamais trouvé).
-- compute_cine_score n'est PAS modifiée.
  ...
  scored as (
    select
      ...
      case
        ...
        when found_attempt between 15 and 20 then 10
        else 0
...
-- Détail d'une séance close (récap par joueur).
...
-- Détail ligue (récap par joueur, par séance).
