-- 018 — Calcul du feedback Ciné'Files entièrement en base (sécurité)
-- Dépend de : 001, 016, 017.
--
-- Remplace le duo « get_cine_target (renvoyait les métadonnées brutes) + calcul
-- TS » par UNE fonction SECURITY DEFINER qui lit la cible en interne, calcule le
-- feedback avec les MÊMES règles que computeFeedback (lib/cinefiles.ts), insère
-- la tentative, et ne renvoie QUE l'objet feedback (jamais tmdb_id/genres/… de
-- la cible). Un joueur ne peut donc plus « peeker » la solution via l'API RPC.

-- get_cine_target n'est plus nécessaire et exposait les métadonnées : on retire.
drop function if exists public.get_cine_target(uuid);

-- Direction d'un critère ordinal (année/décennie) : la CIBLE relativement au
-- film deviné. Identique à `direction()` en TS.
create or replace function public._cine_dir(_target int, _guess int)
returns text
language sql
immutable
as $$
  select case
    when _target is null or _guess is null then 'unknown'
    when _target = _guess then 'exact'
    when _target < _guess then 'earlier'
    else 'later'
  end;
$$;

-- Soumet une tentative : valide les règles, lit la cible (definer), calcule le
-- feedback, insère la ligne, renvoie le feedback. `_guess` = métadonnées du film
-- DEVINÉ : { genres[], releaseDate, director, country, originalLanguage,
-- castNames[], platforms[] }.
...
begin
  ...
  -- Mêmes garde-fous que la policy INSERT des tentatives.
  ...
  -- Intersections insensibles à la casse (casse d'origine du guess conservée).
  ...
  -- Genre : none (aucun partagé) / exact (mêmes ensembles) / partial.
